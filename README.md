...
- **Quire Draft (optional)** — A thoughtful writing room. Bring your own provider key—direct, through OpenRouter, or via an OpenAI-compatible API—to refine a selected passage or create a complete LaTeX draft from a brief. Curated exact model IDs, provider catalog loading, and a custom-model option keep the choice yours; every result stays reviewable before it changes a file.
